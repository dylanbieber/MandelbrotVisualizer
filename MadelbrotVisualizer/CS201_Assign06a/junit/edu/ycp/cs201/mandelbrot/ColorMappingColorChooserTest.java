...
public class ColorMappingColorChooserTest {
	...
	public void testCreateIterSpectrumMap() {
		...
		// test for equality of all occurrences for those keys
		for (Integer key : testKeySet) {
			System.out.println("value: " + testIterSpectrumMap.get(key) + "  " + testIterSpectrumMap.get(key));
			assertTrue((int)testIterSpectrumMap.get(key) == (int)testIterSpectrumMap.get(key));
		}	
...
System.out.println("*** createIterColorMap Test ***");
		...
		// # of entries in each Map must be the same
		assertTrue(testIterCountMap.size() == iterColorMap.size());
		
		// get key sets and place in arrays
		Set<Integer> testKeySet = iterColorMap.keySet();
		...
		// test for equality of all occurrences for those keys
		for (Integer key : testKeySet) {
			System.out.println("value: " + testIterColorMap.get(key) + "  " + testIterColorMap.get(key));
			assertTrue(testIterColorMap.get(key).equals(testIterColorMap.get(key)));
		}
